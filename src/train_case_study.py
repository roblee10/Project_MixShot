...
def main():
    ...
    if args.do_meta_train:
        ...
        train_loader = get_dataloader('train', not args.disable_train_augment, shuffle=True, out_name=True)
    ...
    train_mean_loader = get_dataloader('train', aug=False, shuffle=False, out_name=False)
...
def meta_evaluate(data, train_mean, out_idx, shot):
    un_list = []
    l2n_list = []
    cl2n_list = []
    ...
    for i in warp_tqdm(range(args.meta_test_iter)):
        train_data, test_data, train_label, test_label, train_idx, test_idx = sample_case(data, out_idx, shot) #25,75(5shot 15 query)
...
def sample_case(ld_dict, out_idx, shot):
...
def do_extract_and_evaluate(model, log):
    ...
    accuracy_info_shot1, episode_shot1 = meta_evaluate(out_dict, out_mean, out_idx, 1)
    accuracy_info_shot5, episode_shot5 = meta_evaluate(out_dict, out_mean, out_idx, 5)\
