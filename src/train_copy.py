...
def main():
    ...
    # for triplet-training
        ...
        triplet_info = [args.triplet_iter,args.triplet_batch, args.triplet_rand]
        train_loader = get_dataloader('train', aug=False, triplet = triplet_info)
    # Training loader
    else:
    ...
    for epoch in tqdm_loop:
        ...
        if args.trp_train:
            triplet_train(train_loader, model, criterion, optimizer, epoch, scheduler, log)
        else: 
        ...
        if args.analyze and (epoch +1) % args.anlz_interval == 0:
            analyze(model, anlz_tr_loader, anlz_val_loader, epoch=epoch, log=log, writer=writer)
...
def analyze(model, anlz_tr_loader,anlz_val_loader, epoch, log, writer):
...
def triplet_train(train_loader, model, criterion, optimizer, epoch, scheduler, log):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i, (input, target) in enumerate(tqdm_train_loader):
        ...
        anchor = output[:args.triplet_batch]
        positive = output[args.triplet_batch : args.triplet_batch*2 ]
        negative = output[args.triplet_batch*2 : args.triplet_batch*3 ]

        # loss = triplet_loss(anchor,positive,negative)
        # Use pytorch TripletLoss module
        trip_loss = nn.TripletMarginLoss().cuda() 
        loss = trip_loss(anchor,positive,negative)
        # loss_CE= criterion(output_FC,target)
        # loss = loss_trip + loss_CE
